...
describe("extension configuration", () => {
  ...
  it("allows localhost only during development", () => {
    ...
    expect(config.apiBaseUrl).toBe("http://localhost:8030");
    expect(config.allowedApiOrigins).toContain("http://localhost:8030");
  ...
  it("builds manifest from host permission policy", () => {
    const manifest = createExtensionManifest({
      ...
      VITE_ALLOWED_API_ORIGINS: "https://api.example.com,http://localhost:8030",
